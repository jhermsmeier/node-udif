...
function Footer() {
  ...
  this.version = 0
  // NOTE: This seems to make more sense as "sectorSize"
  this.headerSize = 0
  this.flags = 0
...
Footer.prototype = {
  ...
  parse: function( buffer, offset ) {
    ...
    if( buffer.length - offset < KolyBlock.size ) {
      throw new Error( `Buffer too small: Expected ${KolyBlock.size} bytes, but got ${buffer.length - offset}` )
    }
