...
/**
 * Block map structure size in bytes
 * NOTE: Size without block entries
 * @const {Number}
 */
BlockMap.size = 204

/**
 * Block entry structure size in bytes
 * @const {Number}
 */
BlockMap.entrySize = 40
...
/**
 * Get the size of a block map in bytes
 * @param {Buffer} buffer
 * @param {Number} [offset=0]
 * @returns {Number} size
 */
BlockMap.getSize = function( buffer, offset ) {
  offset = offset || 0
  return buffer.readUInt32BE( offset + 200 ) *
    BlockMap.entrySize + BlockMap.size
}
...
BlockMap.prototype = {
  ...
  parse: function( buffer, offset ) {
    ...
    var size = BlockMap.getSize( buffer, offset )
    if( size !== ( buffer.length - offset ) ) {
      throw new Error( `Buffer too small: Expected ${size} bytes, but got ${buffer.length - offset}` )
    }
