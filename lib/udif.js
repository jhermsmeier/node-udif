...
/**
 * Default block size in bytes (1MB)
 * @type {Number}
 * @constant
 */
UDIF.BLOCK_SIZE = 1024 * 1024
...
/**
 * Compress an image block
 * @param {UDIF.BLOCK} type
 * @param {Buffer} buffer
 * @param {Function} callback(error,buffer)
 */
UDIF.compressBlock = function( type, buffer, callback ) {
  switch( type ) {
    case UDIF.BLOCK.UDZO:
      zlib.deflate( buffer, callback )
      break
    case UDIF.BLOCK.UDBZ:
      callback( new Error( 'Unsupported compression method: BZIP2' ) )
      break
    case UDIF.BLOCK.UDCO:
      callback( new Error( 'Unsupported compression method: ADC' ) )
      break
    case UDIF.BLOCK.LZFSE:
      callback( new Error( 'Unsupported compression method: LZFSE' ) )
      break
