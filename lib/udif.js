 ...
 */
UDIF.decompressBlock = function( type, buffer, length, callback ) {
  ...
  try {
    switch( type ) {
      case UDIF.BLOCK.UDZO: callback( null, zlib.inflateSync( buffer ) ); break
      case UDIF.BLOCK.UDBZ: callback( null, bunzip.decode( buffer ) ); break
      case UDIF.BLOCK.UDCO: callback( null, adc.decompress( buffer.slice( 0, length ) ) ); break
      case UDIF.BLOCK.LZFSE: callback( new Error( 'Unsupported compression method: LZFSE' ) ); break
      default: callback( new Error( 'Unknown compression method for type "0x' + type.toString(16) + '"' ) ); break
    }
  } catch( error ) {
    callback( error )
  }
