...
Block.prototype = {
  ...
  write( buffer, offset ) {
    ...
    offset = offset || 0
    buffer = buffer || Buffer.alloc( Block.size + offset )

    buffer.writeUInt32BE( this.type, offset + 0 )
    buffer.write( this.comment, offset + 4, 4, 'ascii' )
    uint64.writeBE( this.sectorNumber, buffer, offset + 8, 8 )
    uint64.writeBE( this.sectorCount, buffer, offset + 16, 8 )
    uint64.writeBE( this.compressedOffset, buffer, offset + 24, 8 )
    uint64.writeBE( this.compressedLength, buffer, offset + 32, 8 )
