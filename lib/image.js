...
Image.prototype = {
  ...
  readPropertyList: function( callback ) {
    ...
    this.fs.read( this.fd, buffer, 0, length, position, ( error, bytesRead, buffer ) => {
      ...
      try {
        data = plist.parse( buffer.toString() ).data
      } catch( error ) {
        return void callback.call( this, error )
      }
  ...
  verifyData( callback ) {
    ...
    if( !this.footer.dataChecksum || this.footer.dataChecksum.type == UDIF.CHECKSUM_TYPE.NONE ) {
    ...
    switch( this.footer.dataChecksum.type ) {
      case UDIF.CHECKSUM_TYPE.CRC32:
        hash = new crc32.Hash({ encoding: 'hex' }); break
      default:
        var error = new Error( `Unknown or unsupported checksum type "${this.footer.dataChecksum.type}"` )
        return void callback.call( this, error )
    }

    var readable = this.fs.createReadStream( null, {
      fd: this.fd,
      autoClose: false,
    ...
    readable.on( 'error', onError )
      ...
      .on( 'readable', function() {
        ...
        if( hadError ) return;
        callback.call( this, null, checksum == this.footer.dataChecksum.value )
      })
